...
const login = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ sub: user._id }, process.env.JWT_SECRET || "", { expiresIn: process.env.JWT_EXPIRATION });
        const _a = user.toJSON(), { contraseña } = _a, data = __rest(_a, ["contrase\u00F1a"]); // separa contraseña del resto de datos de usuarios(los extrae)
        return res.header("auth-token", token).json(Object.assign(Object.assign({}, data), { token })); //mando en la cabecera el token y en el json el usuario(sin password) y el token
    }
    catch (error) {
        next(error);
    }
